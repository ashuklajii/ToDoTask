...
export default function Page() {
  ...
  return (
    ...
    >
      ...
      <div
        className="
          w-full sm:w-[414px] sm:h-[640px]
          border-4 border-dark rounded-3xl 
          bg-dark relative overflow-hidden
        "
      >
        {/* 🔍 SEARCH BAR (only when not opened) */}
        {!activeCategory && (
          <div className="w-full pt-3 px-2 animate-in fade-in-0">
            <SearchBar onSearch={setSearch} onAddCategory={handleAddCategory} />
          </div>
        )}

        {/* CATEGORY LIST */}
        <div
          className={`mt-3 px-1 pb-10 space-y-3 transition-all duration-300 ${
